  ...
  return <section id="hero" className="min-h-screen px-3 py-3 sm:px-5 sm:py-5">
    <motion.div className="relative mx-auto h-[calc(100vh-24px)] min-h-[660px] max-w-[1460px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#080808] shadow-[0_60px_180px_rgba(0,0,0,.8)] sm:h-[calc(100vh-40px)] sm:rounded-[2.7rem]" initial={{ opacity: 0, scale: .985 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.1, ease }}>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 grid items-end gap-5 p-5 sm:p-8 xl:grid-cols-[minmax(0,1fr)_minmax(270px,390px)] lg:p-10">
        <motion.h1 className="min-w-0 max-w-full whitespace-nowrap text-[clamp(3.4rem,13vw,12rem)] font-extrabold leading-[.72] tracking-[-.105em] text-[#e8e7d5] drop-shadow-2xl" initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: .25, ease }}>{content.hero.title}</motion.h1>
...
function About({ content, lang }: { content: SiteContent; lang: Lang }) {
  return <section id="about" className="container py-20 md:py-28"><Reveal><div className="panel mx-auto max-w-4xl rounded-[2rem] px-7 py-16 text-center md:rounded-[2.5rem] md:px-14 md:py-24">
    <p className="mb-8 text-[10px] font-bold uppercase tracking-[.35em] text-[#e1e0cc]/45">{t(content.about.label, lang)}</p>
    <h2 className="mx-auto max-w-3xl text-balance text-[clamp(2.2rem,5.2vw,4.6rem)] font-bold leading-[.94] tracking-[-.07em]"><WordsPullUpMultiStyle segments={[{ text: t(content.about.prefix, lang) }, { text: t(content.about.accent, lang), className: "accent mx-2 text-[1.08em] font-medium" }, { text: t(content.about.suffix, lang) }]} /></h2>
...
function Tools({ content, lang }: { content: SiteContent; lang: Lang }) {
  return <section id="tools" className="border-y border-white/5 bg-[#050505] py-24"><div className="container">
    <Reveal><div className="mb-10 text-center"><h2 className="text-[clamp(2rem,4vw,3.2rem)] font-bold tracking-[-.06em]">{t(content.toolsIntro.title, lang)}</h2><p className="text-xl text-[#e1e0cc]/50">{t(content.toolsIntro.subtitle, lang)}</p></div></Reveal>
...
function Collaborators({ content, lang }: { content: SiteContent; lang: Lang }) {
  ...
  return <section id="collaborators" className="container py-24"><div className="panel rounded-[2.2rem] p-5 md:p-10 lg:p-16">
    <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr]"><Reveal><div className="lg:sticky lg:top-10"><p className="mb-5 text-[10px] font-bold uppercase tracking-[.35em] text-[#e1e0cc]/45">{t(content.collaboratorsIntro.eyebrow, lang)}</p><h2 className="text-[clamp(2.5rem,5.8vw,5rem)] font-bold leading-[.86] tracking-[-.07em]"><span>{t(content.collaboratorsIntro.titlePrefix, lang)} </span><span className="accent block text-[1.08em]">{t(content.collaboratorsIntro.titleAccent, lang)}</span></h2><p className="mt-8 max-w-sm text-sm leading-7 text-[#e1e0cc]/55">{t(content.collaboratorsIntro.body, lang)}</p></div></Reveal>
...
function CounterStat({ stat, lang }: { stat: SiteContent["stats"][number]; lang: Lang }) {
  ...
  return <div ref={ref} className="border-white/10 text-center sm:border-r last:border-r-0"><div className="text-5xl font-light tracking-[-.06em]">{n}{stat.suffix}</div><p className="mt-3 text-sm font-bold">{t(stat.label, lang)}</p><p className="mx-auto mt-3 max-w-[12rem] text-xs leading-5 text-[#e1e0cc]/42">{t(stat.description, lang)}</p></div>;
}
...
export default function Home() {
  ...
  return <main className="relative"><Hero content={content} lang={lang} setLang={setLang} openAdmin={() => setAdminOpen(true)} /><About content={content} lang={lang} /><Tools content={content} lang={lang} /><Collaborators content={content} lang={lang} /><Stats content={content} lang={lang} /><Contact content={content} lang={lang} /><footer className="container border-t border-white/10 py-10 text-sm text-[#e1e0cc]/45">{lang === "ru" ? "© 2026 alex.creates — кинематографичный монтаж и режиссура." : "© 2026 alex.creates — cinematic editing and direction."}</footer><AdminPanel open={adminOpen} onClose={() => setAdminOpen(false)} content={content} setContent={setContent} /></main>;
}
